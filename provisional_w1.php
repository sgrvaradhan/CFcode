...
<body>
    <div>
    <?php 
$result2 = mysqli_query($connect,"SELECT DISTINCT `Centre_code` FROM provisional where Weeks = 'W1'"); #fetch provisional candidate centres              
while($row2 = mysqli_fetch_array($result2)) 
{ 
    $gc_code = $row2['Centre_code'];
    ...
    ?>
    <table id="headtb" >
        <tr><td rowspan="2"><h2 style="text-align:center;">GATE 2025</h2></td><td><h3  style="text-align:center;">GATE 2025 Provisional Candidates List - Week 1 </h3></td></tr>
    ...
    <?php      
        $i=1;
        $fetchstud = mysqli_query($connect,"SELECT * FROM provisional where Centre_code ='$gc_code' and Weeks = 'W1'"); #fetch provisional candidate
        while($row3 = mysqli_fetch_array($fetchstud))
        { ?> 
            <tr><td style="text-align: center;"><?php echo $i; ?></td>
            <td><?php echo $row3["Registration_No"]; ?></td>
